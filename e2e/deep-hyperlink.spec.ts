...
const getCellData = async (page: Page, row: number, col: number): Promise<{
  ...
  return await page.evaluate(([r, c]) => {
    ...
    return { content: cell?.content, hyperlink: cell?.hyperlink ?? null };
...
const clickContextMenuItem = async (page: Page, label: string): Promise<void> => {
  ...
  await menu.locator('.cell-context-menu-item', { hasText: label }).click();
};
...
test.describe('超链接 - 删除流程', () => {
  ...
  test('移除超链接后内容应保留但超链接数据应清除', async ({ page }) => {
    ...
    expect(data.hyperlink).not.toBeNull();
    ...
    expect(data.hyperlink).toBeNull();
